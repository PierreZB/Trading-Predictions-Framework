...
for datesLoad in datesLoadList:
    for instrument in instruments:
        for granularity in granularities:
            ...
            if df_rawCount > 0:
                ...
                df_parsed['timestamp']      = pd.to_datetime(pd.Series(df_parsed['time']), format="%Y-%m-%dT%H:%M:%S.000000000Z")
                df_parsed['dateYYYMMDD']    = df_parsed['timestamp'].dt.strftime('%Y%m%d')
                df_parsed['timeHHMMSS']     = df_parsed['timestamp'].dt.strftime('%H%M%S')
                ...
                df_parsed = df_parsed.rename(index = str, columns = {"o": "open", "h": "high", "l": "low", "c": "close"})
                df_parsed = df_parsed[[
                    'instrument',
                    'granularity',
                    'timestamp',
    ...
    '''
                df_rawConcat = pd.concat(
                    [df_rawConcat, df_parsed], ignore_index=True, sort=False
                )
