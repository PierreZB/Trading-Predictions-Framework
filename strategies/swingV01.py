...
for strategyFile in strategyFileList:
    ...
    # df['highShifted'] = df['high'].shift(-periodsToAnalyseAfterCandle)
    # df['lowShifted'] = df['low'].shift(-periodsToAnalyseAfterCandle)
    #
    # df['maxHighNextP'] = df['highShifted'].rolling(
    #     window=periodsToAnalyseAfterCandle, min_periods=0
    # ).max()
    #
    # df['minLowNextP'] = df['lowShifted'].rolling(
    #     window=periodsToAnalyseAfterCandle, min_periods=0
    # ).min()
    ...
    df['buyingSignalToShift'] = np.where(
        ((df['currentCandleOC'] == 1) &
         ...
         (1 - (df['minLowNextP'] / df['open']) < maxSLPerThsd)
         ),
        1,
        0
    )
    ...
    df['buyingSignalTmp'] = np.where(
        ((df['currentCandleOC'] == 0) &
         ...
         (1 - (df['minLowNextP'] / df['close']) < maxSLPerThsd)
         ),
        1,
        0
    )
    ...
    df['sellingSignalToShift'] = np.where(
        ((df['currentCandleOC'] == 0) &
         ...
         ((df['maxHighNextP'] / df['open']) - 1 < maxSLPerThsd)
         ),
        1,
        0
    )
    ...
    df['sellingSignalTmp'] = np.where(
        ((df['currentCandleOC'] == 1) &
         ...
         ((df['maxHighNextP'] / df['close']) - 1 < maxSLPerThsd)
         ),
        1,
        0
    )
